...
# ╔═╡ af7b7f96-d5fe-4b16-9b0a-015fcd4e906f
using CairoMakie, PlutoUI

# ╔═╡ 0051014c-5833-11ed-2ddd-49450c989110
md"""
# A nice approximate of the norm of a 2D vector.
...
# ╔═╡ 75e677e6-abad-44a0-bdee-3ecce69f623f
TableOfContents()

# ╔═╡ 02c8511f-3168-4437-a4b8-090cc59bf66a
md"""# Setting-up the scene.
...
# ╔═╡ 5a82e14d-b0a3-4434-b99a-42f18a72ae63
θ = 0:0.01:2π

# ╔═╡ 42fb698a-b563-4db2-ba94-6da685428bf9
x,y = cos.(θ), sin.(θ);

# ╔═╡ afa1efa9-d280-4567-98ea-e135336235e7
let
	f = Figure()
	ax = Axis(f[1,1], 
		xlabel="x", ylabel="y",
		...
		#xminorticks=[-√2, -1, -cos(π/8), -cos(π/4), -cos(3π/8), cos(3π/8), cos(π/4), cos(π/8), 1, √2],
		#yminorticks=[-√2, -1, -cos(π/8), -cos(π/4), -cos(3π/8), cos(3π/8), cos(π/4), cos(π/8), 1, √2]
	)
	hidespines!(ax)
...
# ╔═╡ feef5bba-e3c9-4762-9746-cfea8ea1fc4d
md"""# Finding a lower bound to the norm.
...
# ╔═╡ a77036ef-2893-4e0f-ae89-1eb5ede4eefc
let
	f = Figure()
	ax = Axis(f[1,1], 
		xlabel="x", ylabel="y",
...
# ╔═╡ b458e31d-43cb-47b2-a7ce-e7e9413307a0
md"""
...
# ╔═╡ 992c35f5-bfdb-4589-9f4c-70cc1a24a64c
let
	f = Figure()
	ax = Axis(f[1,1], 
		xlabel="x", ylabel="y",
...
# ╔═╡ b792afde-d9a4-49ba-af8b-3dc8102eff97
md"""We have a higher bound! By choosing the closest to the circle between the yellow and green curves, we get an octagon that is very close to the circle. We can define the upper bound of the circle with a function $f$ such as:
...
# ╔═╡ a645e128-8587-4354-9a15-9d1963b4fb27
let
	f = Figure()
	ax = Axis(f[1,1], 
		xlabel="x", ylabel="y",
...
# ╔═╡ e1ed9cb6-50c7-45bd-8ca9-5bd7306ce949
md"""# Finding an upper bound to the norm.
...
Thus the lower-bound for the 2-norm with the octagon method is $\sqrt{4 - 2\sqrt{2}}f(x,y)$:
...
# ╔═╡ 9a9dda8e-579d-4c6a-88ac-7ce6dcf41b3b
let
	f = Figure()
	ax = Axis(f[1,1], 
		xlabel="x", ylabel="y",
...
# ╔═╡ 4bd12014-978e-4878-9e83-f1e3e7cc916e
md"""
...
# ╔═╡ 8c391c4f-739b-42d0-b742-3252a0af54fd
f(x,y) = max(max(abs(x),abs(y)), (abs(x) + abs(y))/√2)

# ╔═╡ d585a836-b52e-4dcb-9c85-0a2e43a423d0
g(x,y,a) = ((1-a) + a*√(4-2√2))*f(x,y)

# ╔═╡ 50dbd2c5-7d51-47e2-bacf-f3b3436f80b5
let
	f = Figure()
	ax = Axis(
		f[1,1],
...
# ╔═╡ 6158653c-1bec-46bb-b5ed-ba8b240c4745
md"""
...
# ╔═╡ eafbce17-491f-43ee-9abc-04d8ec4a41e3
let
	f = Figure()
	ax = Axis(f[1,1], 
		xlabel="x", ylabel="y",
...
# ╔═╡ 6c174bc1-c19a-44fd-a8b5-6aeba6c2bb0c
md"""To minimize the error with our approximation, we want to minimize the square error. That is:
...
# ╔═╡ 7150c5f8-14d9-4387-80c6-6003f74ec82c
h(a) = (1-a + a*√(4-2√2))

# ╔═╡ 48655615-4b02-4864-a3f9-26e0e5f32cd5
let
	f = Figure()
	ax = Axis(
		f[1,1],
...
# ╔═╡ c475ccf8-2420-424b-aa68-a376470e306c
let
	f = Figure()
	ax = Axis(
		f[1,1],
...
# ╔═╡ b244b356-e6c6-4c8e-a5fb-347ecc4c8ffb
md"""
As we can see from the plots, there is a minimal error, and though 0.5 is a reasonable choice for $a$, we can do slightly better around 0.3.
"""

# ╔═╡ ae625a17-6cc1-4c66-b381-38e8b0a18d9c
md"""We can explicitely calculate $e(a)$. Let $h(a)=(1+a(A-1))$. We have
...
# ╔═╡ 6d0255a2-2a30-4792-8678-0f5149b783b3
a = let
	B = π/16 + 1/(4√2)
	A = √(4-2√2)
	(A/(2B*√2)-1)/(A-1)
...
# ╔═╡ ac0d0231-d58b-4390-afee-727ad9dbafec
ha = h(a)

# ╔═╡ 5138cf5a-545a-4511-8e94-f46bd1d2796f
max_dev = max(h(a)-1, abs(h(a)*cos(π/8)-1))

# ╔═╡ 4b7d8da8-1821-4096-a496-03872b143add
md"""The maximum deviation from the result is then $\max_\theta{|h(a)\cos\theta-1|}$. Looking for that maximum is like looking for the maximum of $\left(h(a)\cos\theta-1\right)^2$. Long story short, the maxima can only occur on the boundaries of the allowed domain for $\theta$, that is $\theta=0$ or $\theta=\pi/8$, meaning
...
# ╔═╡ 53799015-b39b-42ab-89ee-241b84b23a9f
let
	f = Figure()
	ax = Axis(
		f[1,1],
...
# ╔═╡ f1a976eb-9682-4d4e-8c52-36ae36482ccd
md"""# Conclusion
...
# ╔═╡ 4be5080e-ee8d-4ea3-be88-efee509f178d
√(2-√2)/(π/8+1/2√2)
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "ce93b55e46303dbb51db61f17a32b38cfc51334a"
